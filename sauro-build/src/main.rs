...
fn main() -> anyhow::Result<()> {
    ...
    let project = Project::new(&args.path, &config)?;
    let packages = project.build(BuildOptions {
        release: args.release,
    })?;
    for pkg in packages {
        pkg.expand(args.output.as_std_path())?;
    }
...
impl<'cfg> Project<'cfg> {
    fn new(
        ...
        let workspace = cargo::core::Workspace::new(&manifest_file, config)?;
...
impl Package {
    fn expand(&self, output: &Path) -> anyhow::Result<()> {
        ...
        let common_prefix = path_common_prefix(&self.sources).with_context(|| {
            format!("missing common prefix for sources of {} package", self.name)
        })?;
    ...
    fn dylib_name(filename: &std::ffi::OsStr) -> anyhow::Result<String> {
        let filename = filename
            .to_str()
            .ok_or_else(|| anyhow::anyhow!("library name is not a valid utf8 string"))?;
    ...
    fn typescript_filename(source_rs: &Path, common_prefix: &Path) -> PathBuf {
...
fn path_common_prefix(p: &[PathBuf]) -> Option<PathBuf> {
    fn common_prefix_impl(p: &Path, q: &Path) -> Option<PathBuf> {
        let p = p.components();
        let q = q.components();

        let mut components = p.zip(q).map_while(|(p, q)| (p == q).then_some(p));
        let first = Path::to_owned(components.next()?.as_ref());
        components
